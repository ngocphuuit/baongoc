...
class WC_flash_sale_Product {
	...
	{
			//add_action( 'woocommerce_product_meta_end', array( $this, 'render_product_messagea' ) );
		/*	*/add_filter( 'woocommerce_grouped_price_html', array(&$this, 'on_price_htmla'), 10, 2 );
			...
			add_action( 'woocommerce_after_shop_loop_item', array( $this, 'render_product_message' ) );			
	}
	public function price_show_title() {
		...
		global $post;
			...
			if ( $discount_price && $discount_price != $base_price ) {
				if ( apply_filters( 'wc_dynamic_pricing_use_discount_format', true ) ) {
					...
					$html = '<del>' .   woocommerce_price($base_price). '</del><ins>' .  woocommerce_price($discount_price). '</ins>';
				} else {
			...
			elseif ( $discount_price === 0 || $discount_price === 0.00 ) {
				$html = $product->get_price_html_from_to( $product->regular_price, __( 'Free!', 'pw_wc_flash_sale' ) );
			}
