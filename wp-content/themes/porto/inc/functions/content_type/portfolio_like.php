...
function porto_ajax_portfolio_like() {
    if (isset($_POST['portfolio_id'])) {
        ...
        echo '<a class="linked" title="' . __('Already Liked', 'porto') . '" data-tooltip><i class="fa fa-heart"></i>'. $like_count . '</a>';
    }
...
function porto_portfolio_like() {
    ...
    if ($like_count && isset($_COOKIE['porto_like_'. $portfolio_id]) ) {
        $output = '<a class="linked" title="' . __('Already liked', 'porto') . '" data-tooltip><i class="fa fa-heart"></i>'. $like_count . '</a>';
    } else {
        $output = '<a class="portfolio-like" title="' . __('Like', 'porto') . '" data-tooltip data-id="' . $portfolio_id . '"><i class="fa fa-heart"></i>'. ($like_count ? $like_count : '0') . '</a>';
    }
