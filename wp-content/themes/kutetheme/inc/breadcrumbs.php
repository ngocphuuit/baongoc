...
class Breadcrumb_Trail {
    ...
    public function __construct( $args = array() ) {
        ...
        $defaults = array(
            'container'       => 'div',
            'separator'       => '<i class="fa-angle-right fa"></i>',
            'before'          => '',
            'after'           => '',
            'show_on_front'   => true,
            'network'         => false,
    ...
    public function trail() {
        ...
        if ( !empty( $this->items ) && is_array( $this->items ) ) {
            ...
            $breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb-trail breadcrumb">'; // xmlns:v="http://rdf.data-vocabulary.org/#" itemprop="breadcrumb"
    ...
    public function do_minute_archive_items() {
        ...
        /* Add the minute item. */
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( esc_attr_x( 'i', 'minute archives time format', 'breadcrumb-trail' ) ) );
    }
    ...
    public function do_hour_archive_items() {
        ...
        /* Add the hour item. */
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( esc_attr_x( 'g a', 'hour archives time format', 'breadcrumb-trail' ) ) );
    }
    ...
    public function do_day_archive_items() {
        ...
        $year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_attr_x( 'Y', 'yearly archives date format',  'breadcrumb-trail' ) ) );
        $month = sprintf( $this->args['labels']['archive_month'], get_the_time( esc_attr_x( 'F', 'monthly archives date format', 'breadcrumb-trail' ) ) );
        $day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( esc_attr_x( 'j', 'daily archives date format',   'breadcrumb-trail' ) ) );
    ...
    public function do_week_archive_items() {
        ...
        $year = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_attr_x( 'Y', 'yearly archives date format', 'breadcrumb-trail' ) ) );
        $week = sprintf( $this->args['labels']['archive_week'],  get_the_time( esc_attr_x( 'W', 'weekly archives date format', 'breadcrumb-trail' ) ) );
    ...
    public function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( esc_attr_x( 'Y', 'yearly archives date format',  'breadcrumb-trail' ) ) ) . '</a>';

                /* If using the %monthnum% tag, add a link to the monthly archive. */
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( esc_attr_x( 'F', 'monthly archives date format', 'breadcrumb-trail' ) ) ) . '</a>';

                /* If using the %day% tag, add a link to the daily archive. */
                elseif ( '%day%' == $tag )
                    $this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( esc_attr_x( 'j', 'daily archives date format', 'breadcrumb-trail' ) ) ) . '</a>';
