...
if (!class_exists('MAD_THEME_HELPER')) {

	class MAD_THEME_HELPER {
		...
		public static function check_page_layout ($post_id = false) {
			...
			if (is_singular()) {
				$result = mad_meta('mad_page_sidebar_position', '', $post_id);
			}
			if (is_404()) {
				$result = 'no_sidebar';
			}
		...
		public static function header_layout () {
			...
			@$header_layout = mad_meta('mad_header_layout', '', $post_id);
		...
		public static function page_layout () {
			...
			@$page_layout = mad_meta('mad_page_layout', '', $post_id);
		...
		public static function main_navigation( $theme_location = 'primary' ) {
			...
			$nav_menu_primary = mad_meta( 'mad_nav_menu', '', mad_post_id() );
			$nav_menu_secondary = mad_meta( 'mad_nav_menu_secondary', '', mad_post_id() );
