			...
			foreach ($posts as $key => $post) {
				...
				if (!empty($thumb)) {
					...
					if (has_post_thumbnail($post->ID)) {
						$image = MAD_THEME_HELPER::get_the_post_thumbnail($post->ID, '60*60', array('title' => esc_attr( $post->post_title ), 'alt' => esc_attr( $post->post_title )));
					}
...
if (!class_exists('mad_widget_social_links')) {
	...
	class mad_widget_social_links extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo MAD_THEME_HELPER::output_html('social_links', $args);
		}
		...
		function form($instance) {
			...
			$args['instance'] = $instance;
			$args['widget'] = $this;
			echo MAD_THEME_HELPER::output_html('social_links_form', $args);
		}
