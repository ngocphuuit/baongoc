	...
	function mad_gallery_shortcode($atts) {
		...
		if (!empty($attachments) && is_array($attachments)) {
			...
			foreach ($attachments as $attachment) {
				if (is_single()) {
					$permalink = MAD_THEME_HELPER::get_post_attachment_image($attachment->ID, '');
					$jackbox = 'jackbox';
				} else {
				...
				$output .= "<div class='item'>";
					$output .= '<a data-group="entry-'. esc_attr($post_id) .'" class="single-image '. esc_attr($jackbox) .' ' . esc_attr($zoom_image) . '" href="'. esc_url($permalink) .'">';
						$output .= MAD_THEME_HELPER::get_the_thumbnail($attachment->ID, $image_size, array( 'class' => 'tr_all_long_hover') );
					$output .= '</a>';
				$output .= "</div>";
			}
			$output .= "</div>";
...
if (!function_exists( 'mad_body_background' )) {
	...
	function mad_body_background() {
		...
		$color = mad_meta('mad_bg_color', '', $post_id);
		$image = mad_meta('mad_bg_image', '', $post_id);
		...
		$image_repeat     = mad_meta('mad_bg_image_repeat', '', $post_id);
		$image_position   = mad_meta('mad_bg_image_position', '', $post_id);
		$image_attachment = mad_meta('mad_bg_image_attachment', '', $post_id);
...
if (!function_exists('flatastic_maps_key_for_plugins')) {

	add_filter( 'script_loader_src', 'flatastic_maps_key_for_plugins', 10 , 99, 2 );

	function flatastic_maps_key_for_plugins ( $url, $handle  ) {
