 ...
 */
class MAD_Thickbox_Image_Field extends MAD_Image_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'mad-thickbox-image', MAD_JS_URL . 'thickbox-image.js', array( 'jquery' ), MAD_VER, true );
	}
	...
	public static function html( $meta, $field ) {
		$i18n_title = apply_filters( 'mad_thickbox_image_upload_string', _x( 'Upload Images', 'image upload', 'flatastic' ), $field );

		// Uploaded images
		$html = parent::get_uploaded_files( $meta, $field );

		// Show form upload
		$html .= "<a href='#' class='button mad-thickbox-upload' data-field_id='{$field['id']}'>{$i18n_title}</a>";
