 ...
 * Map field class.
 */
class MAD_Map_Field extends MAD_Field {
	...
	static function admin_enqueue_scripts() {
		 ...
		 */
		$google_maps_url = apply_filters( 'mad_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_style( 'mad-map', MAD_CSS_URL . 'map.css' );
		wp_enqueue_script( 'mad-map', MAD_JS_URL . 'map.js', array( 'jquery-ui-autocomplete', 'google-maps' ), MAD_VER, true );
	}
	...
	static function html( $meta, $field ) {
		$html = '<div class="mad-map-field">';

		$html .= sprintf(
			'<div class="mad-map-canvas" data-default-loc="%s"></div>
			<input type="hidden" name="%s" class="mad-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
		...
		if ( $address = $field['address_field'] ) {
			$html .= sprintf(
				'<button class="button mad-map-goto-address-button" value="%s">%s</button>',
				is_array( $address ) ? implode( ',', $address ) : $address,
				esc_html__( 'Find Address', 'flatastic' )
			);
		}
	...
	static function the_value( $field, $args = array(), $post_id = null ) {
		...
		/*
		 * Enqueue scripts
		 * API key is get from $field (if found by MAD_Helper::find_field()) or $args as a fallback
		...
		$google_maps_url = apply_filters( 'mad_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_script( 'mad-map-frontend', MAD_JS_URL . 'map-frontend.js', array( 'google-maps' ), '', true );
		...
		$output = sprintf(
			'<div class="mad-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
