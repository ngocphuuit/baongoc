 ...
 */
class MAD_Taxonomy_Field extends MAD_Object_Choice_Field {
	...
	public static function normalize( $field ) {
		...
		if ( empty( $field['placeholder'] ) ) {
			$field['placeholder'] = esc_html__( 'Select a term', 'flatastic' );
			if ( is_string( $field['taxonomy'] ) && taxonomy_exists( $field['taxonomy'] ) ) {
				$taxonomy_object      = get_taxonomy( $field['taxonomy'] );
				$field['placeholder'] = sprintf( __( 'Select a %s', 'flatastic' ), $taxonomy_object->labels->singular_name );
			}
		}
