 ...
 */
class MAD_File_Field extends MAD_Field {

	/**
	 * Enqueue scripts and styles
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'mad-file', MAD_CSS_URL . 'file.css', array(), MAD_VER );
		wp_enqueue_script( 'mad-file', MAD_JS_URL . 'file.js', array( 'jquery' ), MAD_VER, true );

		self::localize_script( 'mad-file', 'madFile', array(
			'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'flatastic' ),
			'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'flatastic' ),
		) );
	}
	...
	public static function add_actions() {
		...
		add_action( 'wp_ajax_mad_delete_file', array( __CLASS__, 'wp_ajax_delete_file' ) );
		add_action( 'wp_ajax_mad_reorder_files', array( __CLASS__, 'wp_ajax_reorder_files' ) );
	}
	...
	public static function wp_ajax_reorder_files() {
		...
		check_ajax_referer( "mad-reorder-files_{$field_id}" );
	...
	public static function wp_ajax_delete_file() {
		...
		check_ajax_referer( "mad-delete-file_{$field_id}" );
		...
		wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'flatastic' ) );
	}
	...
	public static function html( $meta, $field ) {
		$i18n_title = apply_filters( 'mad_file_upload_string', _x( 'Upload Files', 'file upload', 'flatastic' ), $field );
		$i18n_more  = apply_filters( 'mad_file_add_string', _x( '+ Add new file', 'file upload', 'flatastic' ), $field );
		...
		$html .= sprintf(
			'<div class="%s">
				...
				<a class="mad-add-file" href="#"><strong>%s</strong></a>
			</div>',
			$classes,
			$i18n_title,
			$field['id'],
	...
	protected static function get_uploaded_files( $files, $field ) {
		$reorder_nonce = wp_create_nonce( "mad-reorder-files_{$field['id']}" );
		$delete_nonce  = wp_create_nonce( "mad-delete-file_{$field['id']}" );

		$classes = 'mad-uploaded';
		if ( count( $files ) <= 0 ) {
			$classes .= ' hidden';
		}
	...
	protected static function file_html( $file ) {
		$i18n_delete = apply_filters( 'mad_file_delete_string', _x( 'Delete', 'file upload', 'flatastic' ) );
		$i18n_edit   = apply_filters( 'mad_file_edit_string', _x( 'Edit', 'file upload', 'flatastic' ) );
		$mime_type   = get_post_mime_type( $file );

		return sprintf(
			'<li id="item_%s">
				<div class="mad-icon">%s</div>
				<div class="mad-info">
					<a href="%s" target="_blank">%s</a>
					<p>%s</p>
					<a href="%s" target="_blank">%s</a> |
					<a class="mad-delete-file" href="#" data-attachment_id="%s">%s</a>
				</div>
			</li>',
			$file,
